...
function Sidebar() {
  ...
  return (
    <>
      <aside
        className={`fixed ${showMenu ? "left-0" : "left-[-5rem] sm:left-0"} top-0 bg-cream shadow-md min-h-full p-2 z-30 flex flex-col justify-between items-center transition-all`}
      >
        <a href="#home">
      ...
      >
        {showMenu ? (
          <IoClose className="relative left-[0.4rem]" />
        ) : (
          <RxHamburgerMenu />
        )}
      </div>
